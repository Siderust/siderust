...
fn main() {
    ...
    println!("  X = {:.8} AU", iss_pos_ecl.x().value());
    println!("  Y = {:.8} AU", iss_pos_ecl.y().value());
    println!("  Z = {:.8} AU", iss_pos_ecl.z().value());
    println!("  Distance from Earth: {:.8} AU (~{:.0} km)\n",
        iss_pos_ecl.distance().value(),
        iss_pos_ecl.distance().value() * 149597870.7
    );

    // Moon's approximate position (geocentric)
    let moon_geo: Position<Geocentric, frames::Ecliptic, Au> = 
        Position::new(0.00257, 0.0, 0.0); // ~384,400 km

    println!("Moon position (Geocentric):");
    println!("  Distance from Earth: {:.5} AU (~{:.0} km)\n",
        moon_geo.distance().value(),
    ...
    println!("  X = {:.6} AU", moon_from_iss.x().value());
    println!("  Y = {:.6} AU", moon_from_iss.y().value());
    println!("  Z = {:.6} AU", moon_from_iss.z().value());
    println!("  Distance from ISS: {:.5} AU (~{:.0} km)\n",
        moon_from_iss.distance().value(),
    ...
    println!("Mars Orbit:");
    println!("  Semi-major axis: {:.3} AU", mars_orbit.semi_major_axis.value());
    println!("  Eccentricity: {:.4}", mars_orbit.eccentricity);
    println!("  Inclination: {:.2}°\n", mars_orbit.inclination.value());
    ...
    println!("  X = {:.6} AU", earth_from_mars.x().value());
    println!("  Y = {:.6} AU", earth_from_mars.y().value());
    println!("  Z = {:.6} AU", earth_from_mars.z().value());
    println!("  Distance: {:.6} AU\n", earth_from_mars.distance().value());
    ...
    println!("  Distance: {:.6} AU\n", earth_from_venus.distance().value());
    ...
    println!("  Distance: {:.6} AU\n", mars_from_venus.distance().value());
    ...
    println!("  X = {:.10} AU", original_earth.x().value());
    println!("  Y = {:.10} AU", original_earth.y().value());
    println!("  Z = {:.10} AU\n", original_earth.z().value());
    ...
    println!("  Distance from Mars: {:.6} AU\n", earth_mars_centric.distance().value());
    ...
    println!("  X = {:.10} AU", recovered_helio.x().value());
    println!("  Y = {:.10} AU", recovered_helio.y().value());
    println!("  Z = {:.10} AU\n", recovered_helio.z().value());

    let diff = ((original_earth.x().value() - recovered_helio.x().value()).powi(2) +
                (original_earth.y().value() - recovered_helio.y().value()).powi(2) +
                (original_earth.z().value() - recovered_helio.z().value()).powi(2)).sqrt();
    println!("Total difference: {:.2e} AU (should be small)\n", diff);

    // =========================================================================
    // 5. Directions from Body-Centric Coordinates
    // =========================================================================
    println!("5. DIRECTIONS FROM SATELLITES");
    println!("------------------------------");

    // Star direction (distant object)
    let star_dir: Direction<Geocentric, frames::Equatorial> = 
        Direction::new(0.707, 0.0, 0.707); // 45° declination, 0° RA

    println!("Star direction (Geocentric):");
    println!("  X = {:.3}", star_dir.x().value());
    println!("  Y = {:.3}", star_dir.y().value());
    println!("  Z = {:.3}\n", star_dir.z().value());

    // View from ISS - direction unchanged for distant stars
    let star_from_iss = star_dir.to_bodycentric(iss_params, jd);
    println!("Star direction from ISS (same for distant objects):");
    println!("  X = {:.3}", star_from_iss.x().value());
    println!("  Y = {:.3}", star_from_iss.y().value());
    println!("  Z = {:.3}\n", star_from_iss.z().value());
    ...
    println!("- Directions of distant objects remain unchanged");
