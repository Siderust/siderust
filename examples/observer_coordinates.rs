...
fn main() {
    ...
    println!("  Longitude: {:.4}° E", greenwich.lon.value());
    println!("  Latitude:  {:.4}° N", greenwich.lat.value());
    println!("  Height:    {:.0} m\n", greenwich.height.value());
    ...
    println!("  Longitude: {:.4}° W", -la_palma.lon.value());
    println!("  Latitude:  {:.4}° N", la_palma.lat.value());
    println!("  Height:    {:.0} m\n", la_palma.height.value());
    ...
    println!("  Longitude: {:.4}° W", -mauna_kea.lon.value());
    println!("  Latitude:  {:.4}° N", mauna_kea.lat.value());
    println!("  Height:    {:.0} m\n", mauna_kea.height.value());
    ...
    println!("  Longitude: {:.4}° E", sydney.lon.value());
    println!("  Latitude:  {:.4}° S", -sydney.lat.value());
    println!("  Height:    {:.0} m\n", sydney.height.value());
    ...
    // Zenith (straight up) - using Greenwich observer
    ...
    println!("Zenith (from Greenwich):");
    println!("  Azimuth:  {:.1}°", zenith.azimuth.value());
    println!("  Altitude: {:.1}° (straight up)\n", zenith.polar.value());

    // North horizon
    let north = spherical::direction::Horizontal::with_site(
        greenwich,
        Degrees::new(0.0),   // On horizon
        Degrees::new(0.0),   // North
    );
    println!("North point on horizon (from Greenwich):");
    println!("  Azimuth:  {:.1}° (North)", north.azimuth.value());
    println!("  Altitude: {:.1}° (horizon)\n", north.polar.value());

    // East, 30° above horizon
    let east_30 = spherical::direction::Horizontal::with_site(
        greenwich,
        Degrees::new(30.0),  // 30° above horizon
        Degrees::new(90.0),  // East
    );
    println!("30° above eastern horizon (from Greenwich):");
    println!("  Azimuth:  {:.1}° (East)", east_30.azimuth.value());
    println!("  Altitude: {:.1}°\n", east_30.polar.value());
    ...
    println!("  X = {:.1} km", satellite_geo.x().value());
    println!("  Y = {:.1} km", satellite_geo.y().value());
    println!("  Z = {:.1} km", satellite_geo.z().value());
    println!("  Distance from Earth center: {:.1} km\n", satellite_geo.distance().value());
    ...
    println!("  X = {:.1} km", satellite_topo.x().value());
    println!("  Y = {:.1} km", satellite_topo.y().value());
    println!("  Z = {:.1} km", satellite_topo.z().value());
    ...
    println!("    Observer lat: {:.2}° N", greenwich.lat.value());
    println!("    Distance: {:.0} km", from_greenwich.distance().value());

    let from_la_palma = object_geo.to_topocentric(la_palma, jd);
    println!("  From La Palma:");
    println!("    Observer lat: {:.2}° N", la_palma.lat.value());
    println!("    Distance: {:.0} km", from_la_palma.distance().value());

    let from_sydney = object_geo.to_topocentric(sydney, jd);
    println!("  From Sydney:");
    println!("    Observer lat: {:.2}° S", -sydney.lat.value());
    println!("    Distance: {:.0} km\n", from_sydney.distance().value());
    ...
    println!("  Height: {:.0} m", sea_level.height.value());
    ...
    println!("  Height: {:.0} m", mountain_top.height.value());
