...
impl Time<JD> {
    ...
    pub fn julian_millennias(&self) -> Millennia {
        Millennia::new(
            (self.quantity - Self::J2000.quantity).value() / Self::JULIAN_MILLENNIUM.value(),
        )
    }
    ...
    pub fn julian_centuries(&self) -> Centuries {
        Centuries::new(
            (self.quantity - Self::J2000.quantity).value() / Self::JULIAN_CENTURY.value(),
        )
    }
    ...
    pub fn julian_years(&self) -> JulianYears {
        JulianYears::new(
            (self.value() - Self::J2000.value()) / Self::JULIAN_YEAR.value(),
        )
    }
