...
///
/// ```
/// use siderust::pod::dynamics::{param_partials_central_diff, Rk4Integrator};
...
/// use qtty::Second;
/// use siderust::time::JulianDate;
/// use siderust::pod::dynamics::variational::{PropagatedArc, VariationalPropagator};
///
/// let s0 = OrbitState::new(
///     JulianDate::new(2_451_545.0).to_j2000s(),
///     Position::<GCRS>::new(7_000.0, 0.0, 0.0),
...
/// use qtty::Second;
/// use siderust::time::JulianDate;
/// use siderust::pod::dynamics::variational::VariationalPropagator;
///
/// let s0 = OrbitState::new(
///     JulianDate::new(2_451_545.0).to_j2000s(),
///     Position::<GCRS>::new(7_000.0, 0.0, 0.0),
        ...
        FM: SiderustAccelerationModel,
    {
        ...
        for _ in 0..n_steps {
            let (new_state, phi_step) =
                principia::propagate_stm(force, state, self.step, ctx)
...
mod tests {
    ...
    fn variational_propagator_accumulates_steps() {
        ...
        let arc = prop
            .propagate(&TwoBody::new(crate::astro::dynamics::GM_EARTH), s, 4, &DynamicsContext::empty())
            .unwrap();
        assert_eq!(arc.len(), 4);
        assert!(!arc.is_empty());
        assert!(arc.final_state().is_some());
    ...
    fn variational_propagator_stm_close_to_identity_for_short_arc() {
        ...
        let arc = prop
            .propagate(&TwoBody::new(crate::astro::dynamics::GM_EARTH), s, 1, &DynamicsContext::empty())
            .unwrap();
    ...
    fn prop_arc_empty_when_zero_steps() {
        ...
        let arc = prop
            .propagate(&TwoBody::new(crate::astro::dynamics::GM_EARTH), s, 0, &DynamicsContext::empty())
            .unwrap();
        assert!(arc.is_empty());
