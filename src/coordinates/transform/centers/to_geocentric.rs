...
mod tests {
    ...
    fn test_bary_to_geo() {
        ...
        let expected_earth_geo =
            Position::<Geocentric, Ecliptic, AU>::new(0.0 * AU, 0.0 * AU, 0.0 * AU);
        assert_cartesian_eq!(
            &earth_geo,
            &expected_earth_geo,
            EPSILON,
    ...
    fn test_icrs_to_gcrs() {
        ...
        let sirius_geocentric_cartesian = barycentric_to_geocentric(
            &sirius_barycentric_cartesian,
            JulianDay::new(2460792.157638889),
        );
        let sirius_geocentric_spherical =
            spherical::Position::<Geocentric, frames::ICRS, AU>::from(&sirius_geocentric_cartesian);
        assert_spherical_eq!(
