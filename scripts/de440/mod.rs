...
pub fn run(data_dir: &Path) -> anyhow::Result<()> {
    ...
    for &(target, center, name) in BODIES {
        ...
        eprintln!("    → {} ({} bytes)", bin_path.display(), std::fs::metadata(&bin_path)?.len());
...
fn generate_rust_module(
    ...
    for (name, meta) in bodies {
        ...
        writeln!(f, "    /// Number of Chebyshev coefficients per coordinate.")?;
        ...
        writeln!(f)?;
        writeln!(f, "    /// 8-byte aligned wrapper for `include_bytes!` data.")?;
        writeln!(f, "    #[repr(C, align(8))]")?;
        writeln!(f, "    struct Aligned([u8; BYTE_COUNT]);")?;
        writeln!(f)?;
        writeln!(f, "    /// Raw coefficient data, embedded with 8-byte alignment.")?;
        writeln!(
            f,
        ...
        writeln!(f)?;
        writeln!(f, "    /// Return the `i`-th record as a slice of `RSIZE` f64 values.")?;
        writeln!(f, "    #[inline]")?;
        ...
        writeln!(f, "        // SAFETY: `DATA` is #[repr(align(8))] so `ptr` is 8-byte aligned.")?;
        writeln!(f, "        // Each record starts at a multiple of RSIZE*8 bytes (also aligned).")?;
        writeln!(f, "        // The binary data was written as native-endian f64 by the build script.")?;
        writeln!(f, "        unsafe {{")?;
        writeln!(f, "            std::slice::from_raw_parts(ptr.add(byte_offset) as *const f64, RSIZE)")?;
        writeln!(f, "        }}")?;
        writeln!(f, "    }}")?;
        writeln!(f, "}}")?;
        writeln!(f)?;
