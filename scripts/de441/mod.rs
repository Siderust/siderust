...
//! DE441 build-time pipeline: download → parse DAF/SPK → extract segments → codegen.
//!
...
///
/// 1. Download `de441_part-2.bsp` (if not cached).
...
pub fn run(data_dir: &Path) -> anyhow::Result<()> {
    ...
    eprintln!("  DE441 BSP at: {}", bsp_path.display());
    ...
    for &(target, center, name) in BODIES {
        let summary = daf
            ...
            .unwrap_or_else(|| {
                panic!(
                    "DE441: segment target={} center={} not found in BSP",
                    target, center
                )
            });
        ...
        assert!(
            ...
            "DE441: only SPK Type 2/3 supported, got Type {}",
            summary.data_type
        );
        ...
        let bin_path = out_dir.join(format!("de441_{}.bin", name));
    ...
    let rs_path = out_dir.join("de441_data.rs");
...
fn generate_rust_module(bodies: &[(&str, spk::SegmentMeta)], path: &Path) -> anyhow::Result<()> {
    ...
    writeln!(f, "// DE441 embedded Chebyshev coefficient data")?;
    writeln!(f)?;
    ...
    for (name, meta) in bodies {
        ...
        writeln!(f, "/// DE441 Chebyshev data for body `{}`.", name)?;
        ...
        writeln!(
            f,
            "    static DATA: &Aligned = &Aligned(*include_bytes!(concat!(env!(\"OUT_DIR\"), \"/de441_{}.bin\")));",
            name
        )?;
        writeln!(f)?;
        writeln!(
        ...
        writeln!(f, "        debug_assert!(i < N_RECORDS, \"DE441 {}: record index {{}} out of range (max {{}})\", i, N_RECORDS - 1);", upper)?;
