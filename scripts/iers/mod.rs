...
fn generate_rust(rows: &[EopRow]) -> Result<String> {
    ...
    writeln!(out, "///")?;
    writeln!(out, "/// All angular quantities are in their native IERS units:")?;
    writeln!(out, "/// pole coordinates in arcseconds, UT1−UTC in seconds,")?;
    ...
    writeln!(out, "    /// Celestial pole offset dX wrt IAU 2000A (milliarcseconds).")?;
    writeln!(out, "    pub dx: f64,")?;
    writeln!(out, "    /// Celestial pole offset dY wrt IAU 2000A (milliarcseconds).")?;
    ...
    for url in &[IERS_URL, USNO_URL] {
        ...
        match client.get(*url).send() {
            Ok(resp) if resp.status().is_success() => {
                ...
                if bytes.len() < 1000 {
                    eprintln!("IERS EOP: response too small ({} bytes), skipping", bytes.len());
                    continue;
                }
