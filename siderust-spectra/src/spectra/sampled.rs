...
#[derive(Debug, Clone)]
pub struct SampledSpectrum<X: Unit, Y: Unit, S: qtty::Scalar = f64> {
...
impl<X: Unit, Y: Unit, S: qtty::Scalar> SampledSpectrum<X, Y, S> {
...
impl<X: Unit, Y: Unit> SampledSpectrum<X, Y, f64> {
    ...
    pub fn integrate(&self) -> Quantity<qtty::Prod<Y, X>, f64>
    where
        Y::Dim: qtty::DimMul<X::Dim>,
        <Y::Dim as qtty::DimMul<X::Dim>>::Output: qtty::Dimension,
    {
        let xs = self.xs_raw();
        let ys = self.ys_raw();
        Quantity::<qtty::Prod<Y, X>, f64>::new(algo::trapz(&xs, &ys))
    }
        ...
        <Y::Dim as qtty::DimMul<X::Dim>>::Output: qtty::Dimension,
    {
        let xs = self.xs_raw();
        let ys = self.ys_raw();
        Quantity::<qtty::Prod<Y, X>, f64>::new(algo::trapz_range(
            &xs,
            &ys,
            lo.value(),
            hi.value(),
    ...
    ) -> Quantity<qtty::Prod<qtty::Prod<Y, Yw>, X>, f64>
    where
        Y::Dim: qtty::DimMul<Yw::Dim>,
        <Y::Dim as qtty::DimMul<Yw::Dim>>::Output: qtty::Dimension,
        <Y::Dim as qtty::DimMul<Yw::Dim>>::Output:
            qtty::DimMul<X::Dim>,
        <<Y::Dim as qtty::DimMul<Yw::Dim>>::Output as qtty::DimMul<
            X::Dim,
        >>::Output: qtty::Dimension,
    {
        let sx = self.xs_raw();
        let sy = self.ys_raw();
        let wx = weight.xs_raw();
        let wy = weight.ys_raw();
        Quantity::<qtty::Prod<qtty::Prod<Y, Yw>, X>, f64>::new(
...
mod tests {
    use super::*;
    use qtty::length::{Meter, Nanometer};
    ...
    fn typed_interp_round_trip() {
        ...
        .unwrap();
        let q = s
            .interp_at(qtty::Quantity::<Nanometer, f64>::new(1.5))
            .unwrap();
    ...
    fn integrate_range_clips_correctly() {
        ...
        .unwrap();
        let lo = qtty::Quantity::<Nanometer, f64>::new(0.5);
        let hi = qtty::Quantity::<Nanometer, f64>::new(3.5);
