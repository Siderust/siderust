...
fn test_spherical_position_heliocentric_roundtrip() {
    let pos = spherical::Position::<centers::Heliocentric, frames::Ecliptic, AstronomicalUnit>::new_raw(
        7.0 * DEG,   // latitude
        120.0 * DEG, // longitude
        1.5 * AU,    // distance
    );
...
fn test_frame_types_roundtrip() {
    ...
    let _: frames::Horizontal = serde_json::from_str(&json_horizontal).expect("deserialize Horizontal");
}
...
fn test_modified_julian_date_roundtrip() {
    ...
    let recovered: ModifiedJulianDate = serde_json::from_str(&json).expect("deserialize modified julian date");
