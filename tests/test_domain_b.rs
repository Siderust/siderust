...
fn roundtrip_center_transform_preserves_position() {
    ...
    let pos_helio = Position::<Heliocentric, Equatorial, Au>::new(
        1000.0, 500.0, -200.0
    );
...
fn astrometric_and_apparent_are_distinct_types() {
    ...
    let astrometric = Astrometric::new(
        spherical::direction::Equatorial::new(180.0 * DEG, 45.0 * DEG)
    );
    ...
    let separation = astrometric.direction().angular_separation(apparent.direction());
...
fn topocentric_roundtrip_preserves_geocentric_position() {
    let geo = Position::<Geocentric, Equatorial, Kilometer>::new(
...
fn topocentric_parallax_negligible_for_stars() {
    ...
    let star_geo = Position::<Geocentric, Equatorial, Au>::new(
        206265.0, 0.0, 0.0
    );
...
fn complete_pipeline_geometric_to_apparent() {
    ...
    let object_helio = Position::<Heliocentric, Equatorial, Au>::new(
        10.0, 0.0, 0.0
    );
